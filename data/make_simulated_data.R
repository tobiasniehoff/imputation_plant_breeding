setwd("M:/My Documents/Master/Masterarbeit/R_thesis_tobias/Folder GitHub/R functions")
source("analysis.R")
source("general_functions.R")
source("BEAGLE_functions.R")
source("simulation_functions.R")
setwd("M:/My Documents/Master/Masterarbeit/R_thesis_tobias/Folder GitHub/data")

# this script will simulate some population history wit =h MoBPS
# starting with pop size 1000 and linearly decreasing to 100
# 3 generations from 100 to 1000
# 10 generations of phenotypic selection
...
set.seed(1)
# simulate breeding program as in Jibrila et al. 2020
n.snps <- 63000
n.qtls <- 2700
n.chr <- 9
ninds <- 1000
...
# removing all SNPs with a minor allele frequency below 0.5
SNP.IDs <- SNP.IDs[AF > 0.01 & AF < 0.99]
...
map.3k <- map[map[,1] %in% SNP.3k,]
map.2k <- map[map[,1] %in% SNP.2k,]
map.1k <- map[map[,1] %in% SNP.1k,]
