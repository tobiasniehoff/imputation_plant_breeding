...
# name_parameter_table <- "API2_example_file" ### MODIFY_HERE: uncomment the line
...
################################################################################
### MOFIFY_HERE: for (i in 1:nrow(para_tab)){
setwd("../data/")
...
if(para_tab$per_family_createlib_createlib[i]) {
  ...
  if(para_tab$per_family_imputation_impute[i]){
    ...
    for(h in 1:length(ls_masked_mt)){
      ...
      # here, HD individuals are removed, if desired
      if(!para_tab$HD_inds_in_gt_set[i]){
        ls_masked_mt[[h]] <- extract_individuals_marker_table(marker_table = ls_masked_mt[[h]], 
                                                              ID_inds = HD_inds, negate = T)
      }
