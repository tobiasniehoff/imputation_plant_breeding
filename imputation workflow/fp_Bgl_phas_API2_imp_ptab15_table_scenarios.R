...
name_parameter_table <- 'fp_Bgl_phas_API2_imp_ptab15_table_scenarios'
...
start.phasing.parameters <- min(which(stringr::str_detect(col.names[1,], pattern = 'per_family_phasing')))

col.names <- 
  c(col.names[1,1:(start.phasing.parameters-1)], 
    paste0(col.names[1,start.phasing.parameters:(start.imputation.parameters-1)], '_phasing'),
    paste0(col.names[1,start.imputation.parameters:ncol(col.names)], '_imputing'))
...
para_tab$per_family_phasing_phasing <- as.logical(para_tab$per_family_phasing_phasing)
para_tab$per_family_imputation_imputing <- as.logical(para_tab$per_family_imputation_imputing)

para_tab$API2_overwrite_imputing <- as.logical(para_tab$API2_overwrite_imputing)
para_tab$founders_imputing <- as.logical(para_tab$founders_imputing)




# if per_family_phasing is 'TRUE' then per_family_imputation cannot be 'FALSE'
# because an individual involved in different families could potentially have 
# two different phase predictions
if(any(!para_tab$per_family_imputation_imputing[para_tab$per_family_phasing_phasing])){
  test <- para_tab[para_tab$per_family_phasing_phasing, 
                   which(colnames(para_tab) %in% c('per_family_imputation_impute', 'trial'))]
  test <- test[!test$per_family_imputation_imputing, ]
  ...
  para_tab$per_family_imputation_imputing[para_tab$per_family_phasing_phasing] <- TRUE
}
################################################################################
...
################################################################################
# first block for creating the haplotype library or phasing

# make arguments for BEAGLE phasing
arguments_phasing <- make_arguments_BEAGLE(gt = "tot_pop.vcf.gz", out = "out_totpop.gt", 
...
arguments_impute <- make_arguments_impute_API2(out = 'imputed_API2'
                                               ...
                                               , n_haplotypes = para_tab$API2_n_haplotypes_imputing[i]
                                               , n_windows = para_tab$API2_n_windows_imputing[i]
                                               , overwrite = para_tab$API2_overwrite_imputing[i]
                                               , error = para_tab$API2_error_imputing[i]
                                               , recomb = para_tab$API2_recomb_imputing[i]
                                               , calling_threshold = para_tab$API2_calling_threshold_imputing[i]
                                               , founders = 'founders_file.txt'
                                               , maxthreads = n_cores
)

# deletes the 'founders' argument if necessary
if(!para_tab$founders_imputing[i]) {
  arguments_impute <- 
...
# it grew like this
if(para_tab$per_family_phasing_phasing[i]){
  ...
  for(k in 1:length(ls_masked_mt)){
    ...
    IN <- impute_familywise_BEAGLE(arguments = arguments_phasing
  ...
  # the below is for the case that no familywise phasing shall be performed (but
  # phasing for the whole population)
  ########
