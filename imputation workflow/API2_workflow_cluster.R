...
# finding the directory of this script
...
n_cores <- as.numeric(Sys.getenv(c("SLURM_CPUS_PER_TASK")))
memory_limit_GB <- as.numeric(Sys.getenv(c("SLURM_MEM_PER_NODE")))/1024

# read in the parameter table
setwd("../input parameter tables")
# name_parameter_table <- "API2_example_file.xlsx"

suppressMessages(
  col.names <- data.frame(readxl::read_excel(paste0(name_parameter_table), sheet = 1, skip = 0))
)
...
suppressMessages(
  para_tab <- data.frame(readxl::read_excel(paste0(name_parameter_table), sheet = 1, skip = 1))
)
...
# read in table to later fill with measurements
suppressMessages(
...
################################################################################
name_parameter_table <- str_remove(name_parameter_table, '.xlsx')
################################################################################
...
################################################################################
# this bit here is for merging the data
