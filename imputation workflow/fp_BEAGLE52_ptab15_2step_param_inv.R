...
name_parameter_table <- 'fp_BEAGLE52_ptab15_2step_param_inv'
...
arguments_phasing <- make_arguments_BEAGLE5.2(gt = "tot_pop.vcf.gz", out = "out_totpop.gt", 
...
arguments_imputing <- make_arguments_BEAGLE5.2(gt = "tot_pop.vcf.gz", out = "out_totpop.gt", 
                                            write_args_to_txt = F, map = "test_map.map"
...
if(para_tab$per_family_imputation_phasing[i] & para_tab$BEAGLE_ref_phased_imputing[i]){
  ...
  for(k in 1:length(ls_masked_mt)){
    ...
    IN <- impute_familywise_BEAGLE5.2(arguments = arguments_phasing
    ...
    # here it is imputed
    IN <- impute_familywise_BEAGLE5.2(arguments = arguments_imputing
