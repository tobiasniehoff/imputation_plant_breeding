...
# name_parameter_table <- "BEAGLE_example_file" ### MODIFY_HERE: uncomment the line
  ...
  if(para_tab$per_family_imputation_imputing[i]) {
    ...
    for(w in 1:length(ls_masked_mt)){
      ...
      if(para_tab$BEAGLE_ref_phased_imputing[i]){
        ...
        # the below is executed if imputation shall be done familywise without prior phasing
        IN <- impute_familywise_BEAGLE(arguments = arguments_imputing
                                       , marker_table = ls_masked_mt[[w]]
                                       , pedigree = ped_fam
                                       , memory_limit_GB = memory_limit_GB
