...
warning(paste0('i is ',i,' and the class is ',class(i)))
...
name_parameter_table <- 'fp_BEAGLE_sgimp_ptab15_table_scenarios'
...
col.names <- c(col.names[1,1:(start.imputation.parameters-1)], 
               paste0(col.names[1,start.imputation.parameters:(start.imputation.parameters+14)], '_imputing'), 
               paste0(
                 col.names[1,(start.imputation.parameters+14+1):(start.imputation.parameters+14+1+14)], '_phasing'))
...
para_tab$who_as_reference <- as.character(para_tab$who_as_reference)
################################################################################
...
arguments_imputing <- make_arguments_BEAGLE(gt = "tot_pop.vcf.gz", out = "out_totpop.gt", 
                                            write_args_to_txt = F, map = "test_map.map"
...
# first, produce a reference library
# reference library has to be produced
# I don't allow per-family reference library anymore because they are more work
# and previous findings showed they yielded worse accuracies
# here it is controlled how many individuals are provided for phasing
if(str_detect(para_tab$offspring_type[i], 
              pattern = 'OP') & str_detect(para_tab$offspring_type[i], 
                                           pattern = 'perfam', negate = TRUE)){
  ped_p <- ped[1:30,]
  hd_o_id <- HD_inds[!HD_inds %in% ped_p[[1]]]
  
  o_id <- c()
  # provide HD offspring
  o_id <- sample(hd_o_id, replace = FALSE, size = ceiling(
    para_tab$fraction_HD_offspring_provided_for_phasing[i]*length(hd_o_id)
  ))
  
  
  # provide LD offspring
  ld_o_id <- extract_individuals_pedigree(pedigree = ped, 
                                          ID_names = HD_inds, 
                                          negate = TRUE)[[1]]
  o_id <- 
    c(o_id,
      sample(ld_o_id, replace = FALSE, 
             size = ceiling(
               para_tab$fraction_LD_offspring_provided_for_phasing[i]*length(ld_o_id)
             ))
    )
  ...
  o_id <- 
    c(o_id,
      extract_random_HD_offspring_per_fam(pedigree = 
                                            extract_individuals_pedigree(pedigree = ped, 
                                                                         ID_names = HD_inds, 
                                                                         negate = TRUE), 
                                          fraction_HD_offspring_per_family = 
                                            para_tab$fraction_LD_offspring_provided_for_phasing[i])
    )
  ...
  o_id <- 
    extract_random_HD_offspring_per_fam(pedigree = 
                                          extract_individuals_pedigree(pedigree = ped, 
                                                                       ID_names = HD_inds), 
                                              fraction_HD_offspring_per_family = 
                                          para_tab$fraction_HD_offspring_provided_for_phasing[i])
...
ped_p <- extract_parents_pedigree(ped)
ped_o <- extract_offspring_pedigree(ped)
# this is kinda hardcoded in here to enable extracting parents for the OP10n100 population
if(str_detect(para_tab$offspring_type[i], 
              pattern = 'OP') & str_detect(para_tab$offspring_type[i], 
                                           pattern = 'perfam', negate = TRUE)){
  ped_p <- ped[1:30,]
  ped_o <- ped[31:nrow(ped),]
}
...
if(para_tab$provide_HD_fam_members_as_ref[i]){
  ref_inds <- unique(c(ref_inds, ped_o[[1]][ped_o[[1]] %in% HD_inds]))
} else{
  ref_phase <- extract_individuals_phase(phase = ref_phase, 
                                         ID_inds = ped_o[[1]][ped_o[[1]] %in% HD_inds], 
                                         negate = TRUE)
}
...
# The above was about the restriction of the reference library

ped_ld <- extract_individuals_pedigree(pedigree = ped, 
                                       ID_names = HD_inds, negate = T)

# looping over all LD individuals
for(h in 1:nrow(ped_ld)){
  # now I will customize the library for every individual
  if(para_tab$who_as_reference[i] == 'whole_library'){custom_reference <- ref_phase}
  if(para_tab$who_as_reference[i] == 'only_actual_parents'){
    custom_reference <- 
      extract_individuals_phase(ref_phase, ID_inds = unlist(ped_ld[h,3:4]))
  }
  if(para_tab$who_as_reference[i] == 'only_parents'){
    custom_reference <- 
      extract_individuals_phase(ref_phase, extract_parents_pedigree(ped)[[1]])
  }
  if(para_tab$who_as_reference[i] == 'only_offspring'){
    custom_reference <- 
      extract_individuals_phase(ref_phase, extract_offspring_pedigree(ped)[[1]])
  }
  if(para_tab$who_as_reference[i] == 'only_1degree_relatives'){
    custom_ref_inds <- 
      extract_family_pedigree(pedigree = ped, family_name = unlist(ped_ld[h,2]))[[1]][[1]]
    custom_reference <- 
      extract_individuals_phase(ref_phase, custom_ref_inds)
  }
  if(para_tab$who_as_reference[i] == 'only_1degree_siblings'){
    custom_ref_inds <- 
      extract_family_pedigree(pedigree = ped, family_name = unlist(ped_ld[h,2]))[[1]]
    custom_ref_inds <- custom_ref_inds[[1]][custom_ref_inds[[2]] != 'Parents']
    custom_reference <- 
      extract_individuals_phase(ref_phase, custom_ref_inds)
  }
  
  # make customized pedigree
  custom_ped <- 
    extract_individuals_pedigree(ped, 
                                 unique(c(unlist(ped_ld[h,1]), 
                                          custom_reference[[1]])))
  custom_ped[[2]] <- custom_ped[[2]][1]
  
  # customize masked_mt
  custom_masked_mt <- 
    extract_individuals_marker_table(masked_mt, ID_inds = ped_ld[[1]][h])
  
  IN <- impute_familywise_BEAGLE(arguments = arguments_imputing
                                 , marker_table = custom_masked_mt
                                 , pedigree = custom_ped
                                 , memory_limit_GB = memory_limit_GB
                                 , mapo = mapo
                                 , verbose = TRUE
                                 , reference_phase = custom_reference)
  
  out_mt_ind <- convert_phase_to_marker_table(IN$out_phase[[1]])
  
  if(h == 1){
    out_mt <- out_mt_ind
    runtime <- run_time + IN$run_time
  } else{
    out_mt <- cbind(out_mt, out_mt_ind[,2])
    runtime <- runtime + IN$run_time
  }
  cat('Finished imputing',round(h/nrow(ped_ld),2), ' needed', IN$run_time, '\n')
}

# now phasing and imputation is finished
# the output only contains ld individuals/individuals not in the reference 
# because it is allowed to analyze for the whole_population, I have to add them
# here
# Also, random missing information is also imputed and must be evaluated
ref_mt <- convert_phase_to_marker_table(ref_phase)
inds_to_add <- 
  colnames(ref_mt[,-1])[!colnames(ref_mt[,-1]) %in% colnames(out_mt[,-1])]

out_mt <- 
  cbind(out_mt, extract_individuals_marker_table(ref_mt, 
                                                 ID_inds = inds_to_add)[,-1])

out_mt <- order_marker_table_by_pedigree(marker_table = out_mt, 
                                         pedigree = colnames(masked_mt[,-1]))
