...
library(stringr)
library(rlist)
suppressMessages(library("Meiosis")) # the 'Meiosis package from Dominik Mueller will be used
library(readxl)

setwd('..')
ls_dirs <- list.dirs(recursive = TRUE)
setwd(ls_dirs[str_detect(ls_dirs, pattern = 'R_thesis_tobias$')])
...
# the scenario this function should run. This allows to impute in parallel
args_script <- commandArgs(trailingOnly = TRUE)
print(args_script)
print(str(args_script))

index_for_scenario <- as.numeric(args_script[1]) 

#i <- as.numeric(Sys.getenv(c("SLURM_ARRAY_TASK_ID"))) # suggestion from Markus Boden
i <- index_for_scenario

# this script used to have a for loop looping over all rows of the parameter table
# and working for all parameter combinations one after another
# this script here is supposed to enable parallel computation
# so a for loop is not needed anymore. 

# the table with parameters must be present in R_thesis_tobias
name_parameter_table <- 'fp_API2_ptab6_LD_for_ref'
...
start.createlib.parameters <- min(which(stringr::str_detect(col.names[1,], pattern = 'per_family_createlib')))

col.names <- 
  c(col.names[1,1:(start.imputation.parameters-1)], 
    paste0(col.names[1,start.imputation.parameters:(start.createlib.parameters-1)], '_impute'),
    paste0(col.names[1,start.createlib.parameters:ncol(col.names)], '_createlib'))
...
para_tab$per_family_imputation_impute <- as.logical(para_tab$per_family_imputation_impute)
para_tab$per_family_createlib_createlib <- as.logical(para_tab$per_family_createlib_createlib)

para_tab$geneticMap_impute <- as.logical(para_tab$geneticMap_impute)

para_tab$API2_overwrite_impute <- as.logical(para_tab$API2_overwrite_impute)
para_tab$API2_overwrite_createlib <- as.logical(para_tab$API2_overwrite_createlib)

para_tab$founders_impute <- as.logical(para_tab$founders_impute)


# if per_family_createlib is 'TRUE' then per_family_imputation cannot be 'FALSE'
# because an individual involved in different families could potentially have 
# two different phase predictions

if(any(!para_tab$per_family_imputation_impute[para_tab$per_family_createlib_createlib])){
  test <- para_tab[para_tab$per_family_createlib_createlib, 
                   which(colnames(para_tab) %in% c('per_family_imputation_impute', 'trial'))]
  test <- test[!test$per_family_imputation_impute, ]
  cat(paste0('Trials number ',test$trial, ' require per family library creation
             but not per_family_imputation which is not possible. Please look at 
             "decision tree API2".\n',
             'per_family_imputation will be set to "TRUE" for these trials.\n'))
  
  para_tab$per_family_imputation_impute[para_tab$per_family_createlib_createlib] <- TRUE
}
################################################################################
# in previous versions, here the simulated data was just read in
# now I read in the filtered data and make a new simulation here every time this script
# is executed
setwd(odir)

ls_dirs <- list.dirs(recursive = TRUE)
setwd(ls_dirs[str_detect(ls_dirs, pattern = "./filtered_data$")])
# setwd("~/Studium Agrarwissenschaften In Goettingen/Master/Masterarbeit/R_thesis_tobias/filtered_data")

map <- read.table('filtered_Sugar-beet-data1_map.txt', header = TRUE)
pedigree <- fread('filtered_Sugar-beet-data1_ped.txt')
imputed_phase <- read_vcf_to_phase("imputed_filtered_pop.gt.vcf.gz", set_missing_gt_to = 9)

map_LD1 <- read.table('filtered_Sugar-beet-data1_map1.txt', header = TRUE)
map_LD2 <- read.table('filtered_Sugar-beet-data1_map2.txt', header = TRUE)
map_LD3 <- read.table('filtered_Sugar-beet-data1_map3.txt', header = TRUE)
setwd(odir)
...
################################################################################
# first block for creating the haplotype library

# create arguments for API2 create library
arguments_createlib <- make_arguments_createlib_API2(out = 'library_API2',
                                                     genotypes = 'masked.genotypes',
                                                     hd_threshold = para_tab$API2_hd_threshold_createlib[i]
                                                     , n_sample_rounds = para_tab$API2_n_sample_rounds_createlib[i]
                                                     , seed = para_tab$seed_createlib[i]
                                                     , n_haplotypes = para_tab$API2_n_haplotypes_createlib[i]
                                                     , n_windows = para_tab$API2_n_windows_createlib[i]
                                                     , overwrite = para_tab$API2_overwrite_createlib[i]
                                                     , error = para_tab$API2_error_createlib[i]
                                                     , recomb = para_tab$API2_recomb_createlib[i]
                                                     , calling_threshold = para_tab$API2_calling_threshold_createlib[i]
                                                     , maxthreads = n_cores
)

# create arguments for API2 imputation
arguments_impute <- make_arguments_impute_API2(out = 'imputed_API2'
                                               , libphase = 'library_API2.phase'
                                               , genotypes = 'masked.genotypes'
                                               , seed = para_tab$seed_impute[i],
                                               , n_haplotypes = para_tab$API2_n_haplotypes_impute[i]
                                               , n_windows = para_tab$API2_n_windows_impute[i]
                                               , overwrite = para_tab$API2_overwrite_impute[i]
                                               , error = para_tab$API2_error_impute[i]
                                               , recomb = para_tab$API2_recomb_impute[i]
                                               , calling_threshold = para_tab$API2_calling_threshold_impute[i]
                                               , founders = 'founders_file.txt'
                                               , maxthreads = n_cores
)
# deletes the 'founders' argument if necessary
if(!para_tab$founders_impute[i]) {
  arguments_impute <- 
    stringr::str_remove(arguments_impute, pattern = ' -founders founders_file.txt')
}
##############################################################################
# in contrast to previous versions of the API2 imputation script, this one here
# does not allow the population for library creation to be different than the 
# population for imputation
...
# Update 10.02.2021: since the individuals are now simulated every time
# this script is executed, it is actually like drawing random individuals
...
################################################################################
# this block is very API2 specific
# since a hd_threshold has to be defined for the individuals that should be provided
# in the library, these individuals have to have higher non-missing marker fractions
# than the threshold
# this is checked here. There are probably still scenarios where these checks here
# can be 'surpassed' but are still causing problems, but these checks should 
# cover the most common scenarios.

a_whole_pop <- analyze_inds_marker_table(masked_mt)
# the condition is asking 'does at least one individual in the whole population
# have at least e.g. 80% of non-missing information?'
if(!any((1-a_whole_pop$freq_NA) >= para_tab$API2_hd_threshold_createlib[i])){
  stop(paste0('No individual has at least a fraction of ',
              para_tab$API2_hd_threshold_createlib[i],
              ' non-missing markers in trial ',i,'. It is therefore not possible to create a library.
              Please use a lower threshold value.'))
}
rm(a_whole_pop)

if(para_tab$provide_parents_as_ref[i]){
  p_id <- extract_parents_pedigree(ped)[[1]]
  
  if(str_detect(offspring_type, 
                pattern = 'OP') & str_detect(offspring_type, 
                                             pattern = 'perfam', negate = TRUE)){
    p_id <- ped[[1]][1:30]
  }
  a_parents <- analyze_inds_marker_table(
    extract_individuals_marker_table(masked_mt, ID_inds = p_id))
  
  if(!any((1 - a_parents$freq_NA) >= para_tab$API2_hd_threshold_createlib[i])){
    stop(paste0('No single parent was detected that has at least ', 
                para_tab$API2_hd_threshold_createlib[i],
                ' non-missing markers in trial ',i,
                ' and are therefore not made available in 
                   the library.\t'))
  }
  rm(a_parents)
} else{
  # is executed if no parents should be provided
  p_id <- extract_parents_pedigree(ped)[[1]]
  off_id <- extract_offspring_pedigree(pedigree = ped, parents_ID = p_id)[[1]]
  
  if(str_detect(offspring_type, 
                pattern = 'OP') & str_detect(offspring_type, 
                                             pattern = 'perfam', negate = TRUE)){
    p_id <- ped[[1]][1:30]
    off_id <- extract_individuals_pedigree(pedigree = ped, ID_names = p_id, negate = TRUE)[[1]]
  }
  a_offspring <- analyze_inds_marker_table(
    extract_individuals_marker_table(masked_mt, ID_inds = off_id))
  
  if(!any((1 - a_offspring$freq_NA) >= para_tab$API2_hd_threshold_createlib[i])){
    stop(paste0('No offspring was detected that has at least ', 
                para_tab$API2_hd_threshold_createlib[i],
                ' non-missing markers in trial ', i,
                ' and are therefore not made available in 
                   the library.\t'))
  }
  rm(off_id); rm(a_offspring)
}
rm(p_id)
################################################################################
...
if(para_tab$per_family_createlib_createlib[i]) {
  # this creates a list of marker tables
  ls_masked_mt <- 
    extract_family_marker_table(pedigree = ped, marker_table = masked_mt)
  
  # now phasing and imputation is done for every family individually
  runtime <- NULL
  for(k in 1:length(ls_masked_mt)){
    ...
    if(nrow(ped_fam) <= 2) {
      cat(paste0('Skipped family ', k,' in trial ',i,' of library creation.\n'))
      next
    }
    ...
    genotypes <- convert_marker_table_to_GenotypeFile(masked_mt_for_phasing)
    gmap <- convert_mapo_to_GeneticMapFileAPI(mapo = mapo)
    
    IN_createlib <- createlib_API2(arguments_createlib = arguments_createlib, 
                                   genotypes = genotypes, 
                                   GeneticMapFile = gmap, verbose = FALSE)
    
    libphase <- IN_createlib[[1]]
    run_time <- IN_createlib[[2]]
    cat(
      paste0('\tFinished creating library for family ',k,' of ',
             length(ls_masked_mt),' families in trial ',i,'.\n')
    )
    
    ############################################################################
    if(nrow(ped_fam) <= 2) {
      cat(paste0('Skipped family ', k,' in trial ',i,' of imputation.\n'))
      next
    }
    ...
    if(para_tab$provide_parents_as_ref[i]){
      ...
      libphase <- extract_individuals_phase(phase = libphase, 
    ...
    if(para_tab$provide_HD_inds_as_ref[i]){
      ref_inds <- unique(c(ref_inds, unlist(libphase[libphase[[1]] %in% HD_inds,1])))
      libphase <- extract_individuals_phase(phase = libphase, ID_inds = ref_inds)
    } else{
      libphase <- extract_individuals_phase(phase = libphase, ID_inds = ref_inds)
    }
    
    founders_obj <- make_founders_obj(pedigree = ped_fam) ############### this here might be a spot for errors
    # it is very well possible that API2 does not accept founders specified in the founders file
    # that are not actually present in the libphase
    genotypes <- convert_marker_table_to_GenotypeFile(ls_masked_mt[[k]])
    gmap <- convert_mapo_to_GeneticMapFileAPI(mapo = mapo)
    
    
    # this will work whether founders are included in the arguments or not
    # so the below is covering two scenarios
    IN_impute <- impute_API2(arguments_impute = arguments_impute, 
                             genotypes = genotypes, 
                             GeneticMapFile = gmap,
                             libphase = libphase, 
                             founders = founders_obj)
    
    imputed_mt <- IN_impute[[1]]
    (run_time <- run_time + IN_impute$run_time)
...
} else{ # else here means in case no familywise library creation shall be performed
  runtime <- NULL
  ...
  cat(
    paste0('\tFinished creating library of trial ',i,'.\n')
  )
  
  # if based on the whole pop library creation, family wise imputation should be performed
  if(para_tab$per_family_imputation_impute[i]){
    ls_masked_mt <- 
      extract_family_marker_table(pedigree = ped, marker_table = masked_mt)
    
    for(h in 1:length(ls_masked_mt)){
      ped_fam <- extract_individuals_pedigree(pedigree = ped, 
                                              ID_names = colnames(ls_masked_mt[[h]]))
      ...
      if(nrow(ped_fam) <= 2) {
        cat(paste0('Skipped family ', h,' in trial ',i,' of imputation.\n'))
        next
      }
      # this is to set the whole pedigree to the same family
      ped_fam[[2]] <- ped_fam[[2]][1]
      
      founders_obj <- make_founders_obj(pedigree = ped_fam) 
      genotypes <- convert_marker_table_to_GenotypeFile(ls_masked_mt[[h]])
      gmap <- convert_mapo_to_GeneticMapFileAPI(mapo = mapo)
      
      # here I am checking which individuals should be given as reference
      libphase_fam <- libphase
      ref_inds <- c()
      if(para_tab$provide_parents_as_ref[i]){
        ref_inds <- ped_fam_p[[1]]
      } else{
        libphase_fam <- extract_individuals_phase(phase = libphase_fam, 
                                              ID_inds = ped_fam_p[[1]], negate = TRUE)
      }
      ...
      if(para_tab$provide_HD_fam_members_as_ref[i]){
        ...
        libphase_fam <- 
          extract_individuals_phase(phase = libphase_fam, 
                                              ID_inds = ped_fam_o[[1]][ped_fam_o[[1]] %in% HD_inds], 
                                              negate = TRUE)
      }
      
      if(para_tab$provide_HD_inds_as_ref[i]){
        ref_inds <- unique(c(ref_inds, unlist(libphase_fam[libphase_fam[[1]] %in% HD_inds,1])))
        libphase_fam <- extract_individuals_phase(phase = libphase_fam, ID_inds = ref_inds)
      } else{
        # this is the final step after all the provide ref questions
        libphase_fam <- extract_individuals_phase(phase = libphase_fam, ID_inds = ref_inds)
      }
      ...
      if(is.null(runtime)){
        out_mt <- imputed_mt
        runtime <- run_time + IN_impute$run_time
      } else{
        out_mt <- cbind(out_mt, imputed_mt[,-1])
        runtime <- runtime + IN_impute$run_time
      }
    }
  } else{ # this 'else' means in case no familywise imputation shall be performed
    
    # not needed to divide to one case with founders and another without founders
    #because the function will take care of it if no founders are
    # specified in the arguments
    founders_obj <- make_founders_obj(pedigree = ped) 
    genotypes <- convert_marker_table_to_GenotypeFile(masked_mt)
    gmap <- convert_mapo_to_GeneticMapFileAPI(mapo = mapo)
    
    # this is for the case of open pollination but not familywise
    if(str_detect(offspring_type, 
                  pattern = 'OP') & str_detect(offspring_type, 
                                               pattern = 'perfam', negate = TRUE)){
      ped_fam <- ped
